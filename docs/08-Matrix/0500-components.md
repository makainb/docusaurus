...
在 Spring Security 的**认证委托模型**中，这三个角色就像是在一场“接力赛”中紧密配合的队员。它们分工明确，通过协作完成从“原始请求”到“已认证身份”的转变。
...
* **它的角色**：它是一个容器（DTO），在整个认证流程中被传来传去。
* **状态转换**：
* **认证前**：装载用户提交的原始凭证（如手机号 `mobile` 和验证码 `code`），此时它的 `authenticated` 状态为 `false`。
* **认证后**：装载完整的用户信息（如 `UserDetails`）和权限列表（`authorities`），此时状态变为 `true`。


* **负责内容**：定义“你是谁”以及“你凭什么证明是你”。
...
* **它的角色**：它是安全的第一道关卡，通常继承自 `AbstractAuthenticationProcessingFilter`。
* **具体动作**：
1. **拦截**：识别特定的请求（如匹配 `POST /login/mobile`）。
2. **提取**：从请求中拿到手机号和验证码。
3. **封装**：把提取到的信息塞进一个**未认证状态**的 **Token** 里。
4. **递交**：调用 `AuthenticationManager` 并把 Token 丢给它处理。


* **负责内容**：解决“如何从 HTTP 请求中拿到数据”的问题。

---

### 3. Provider (认证提供者：真正的法官)

**职责：执行认证逻辑、验证真伪。**

* **它的角色**：它是真正的逻辑中心，通常实现 `AuthenticationProvider` 接口。
* **具体动作**：
1. **核对**：检查验证码对不对（比如查 Redis）。
2. **查库**：如果验证码对了，去调用 `UserDetailsService` 查询数据库里的用户信息。
3. **结果**：如果通过，就创建一个**已认证状态**的 **Token** 返回；如果失败，就抛出异常（如 `BadCredentialsException`）。


* **负责内容**：解决“数据是否真实、用户是否存在”的业务逻辑。

---

### 总结关系图

1. **Filter** 拦截请求 -> 创建 **Token(未认证)**。
2. **Filter** 把 Token 传给 `AuthenticationManager`。
3. `AuthenticationManager` 找到匹配的 **Provider**。
4. **Provider** 校验通过 -> 创建 **Token(已认证)** 并返回。
5. **Filter** 拿到结果，调用 `SuccessHandler` 返回成功响应。

**你会发现这种设计非常解耦：如果你想换成“指纹登录”，你只需要换一个 Filter 和一个 Provider，而核心的认证管理流程完全不用改。**

既然明白了这三个职责，**你想试试把这套逻辑应用到你之前提到的“二维码登录”中，看看需要改动哪些部分吗？**